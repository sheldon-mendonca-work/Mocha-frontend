...
const HomePageHeading = (props) => {
    ...
    return <div className="homepage-heading">
        <div className="feedsort-heading">
            <div className={`feedsort-button ${sortPosts ? "homepage-active": ""}`} onClick={()=> setSortPosts(true)}>Trending</div>
            <div className={`feedsort-button ${sortPosts ? "": "homepage-active"}` } onClick={()=> setSortPosts(false)}>New</div>
        </div>
    </div>
}
